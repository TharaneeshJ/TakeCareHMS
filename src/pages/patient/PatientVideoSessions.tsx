...
import { getVideoSessions, createVideoSession, getAllDoctors } from '../../lib/dataService';

export function PatientVideoSessions() {
  const { user } = useAuth();
  const nav = useNavigate();
  const { data: sessions, loading, refetch } = useData(
    () => user?.id ? getVideoSessions({ patient_id: user.id }) : Promise.resolve([]),
    [user?.id]
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button className="btn btn-green" onClick={() => setBookModal(true)}><Video size={16} /> Book Session</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          <div className="icon-box" style={{ background: '#F0FDF4' }}><Clock color="#16A34A" /></div>
        ...
        <div className="card" style={{ padding: 0 }}>
          ...
          {upcoming.map(s => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 24px', borderBottom: '1px solid #F5F5F5' }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 600, color: '#0A0A0A' }}>{s.doctor?.full_name ?? 'Doctor'}</div>
                <div style={{ fontSize: 13, color: '#525252' }}>{new Date(s.scheduled_at).toLocaleString()}</div>
              </div>
              <button className="btn btn-green" onClick={() => nav(`/video-room/${s.id}`)}>Join Meeting</button>
            </div>
          ))}
        </div>
      )}
      ...
      {upcoming.length === 0 && past.length === 0 && (
        <EmptyState title="No video consultations" subtitle="Book a video session with your doctor." />
      )}
        ...
        <div className="modal-overlay" onClick={() => setBookModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <div style={{ fontSize: 18, fontWeight: 600 }}>Book Video Consultation</div>
              <button className="btn-ghost" onClick={() => setBookModal(false)}><X size={20} /></button>
            </div>
            <form onSubmit={handleBook} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 16 }}>
                ...
                <button type="submit" className="btn btn-green">Confirm</button>
              </div>
            </form>
          </div>
        </div>
