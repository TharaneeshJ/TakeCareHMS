...
import { getAppointments, createAppointment, getAllDoctors, cancelAppointmentSafe } from '../../lib/dataService';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button className="btn btn-green" onClick={() => setBookModal(true)}><Calendar size={16} /> Book Appointment</button>
      </div>
            ...
            <tbody>
              {(appointments ?? []).map(a => (
                <tr key={a.id}>
                  ...
                  <td>
                    {a.status === 'scheduled' && (
                      <button className="btn-ghost" style={{ color: '#DC2626', fontSize: 12, padding: '4px 8px' }} onClick={() => handleCancel(a.id)}>Cancel</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="modal-overlay" onClick={() => setBookModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <div style={{ fontSize: 18, fontWeight: 600 }}>Book Appointment</div>
            ...
            <form onSubmit={handleBook} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div><label className="input-label">Appointment Type</label>
                <select name="type" className="input" required>
                  <option>Consultation</option><option>Follow-up</option><option>Check-up</option><option>Video Consultation</option>
                </select>
              </div>
